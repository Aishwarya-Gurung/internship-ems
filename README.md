# Screen Shots

## Department

### Post department
![task1](output/postdepartment.PNG)
### Get all department
![task1](output/getalldepartment.PNG)
### Get department by id
![task1](output/getbyiddepartment.PNG)
### Put department
![task1](output/putdepartment.PNG)
### Delete department
![task1](output/deletedepartment.PNG)


### Post Salary
![task1](output/postsalary.PNG)
### Get all Salary
![task1](output/getallsalary.PNG)
### Get Salary by id
![task1](output/getbyidsalary.PNG)
### Put Salary
![task1](output/putsalary.PNG)
### Delete Salary
![task1](output/deletesalary.PNG)

### Post Project
![task1](output/postproject.PNG)
### Get all Project
![task1](output/getallproject.PNG)
### Get Project by id
![task1](output/getbyidproject.PNG)
### Put Project
![task1](output/updateproject.PNG)
### Delete Project
![task1](output/deleteproject.PNG)

### Post Employee
![task1](output/postemployee.PNG)
### Get all Employee
![task1](output/getallemployee.PNG)
### Get Employee by id
![task1](output/getbyidemployee.PNG)
### Get Employee by designation
![task1](output/getbydesignationemployee.PNG)
### Put Employee
![task1](output/updateemployee.PNG)
### Delete Employee
![task1](output/deleteemployee.PNG)

